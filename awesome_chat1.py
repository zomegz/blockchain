...
def chitchat(messages, openaikey=None):
    ...
    # First, send the message to the OpenAI API
...
def chat_huggingface(messages, openaikey = None, return_planning = False, return_results = False):
 ...
 # At the end, check if the response_text contains a request to create, save or append to a text file
    file_related_request = re.search(r"(create|save|append)\s+(a|to)\s+(text|txt)\s+file", response_text, re.IGNORECASE)
    if file_related_request:
        # Determine the file name and the content to save or append
        file_name = "your_file_name.txt"  # You can extract the desired file name from the response_text or generate it
        content_to_save = "your_content_to_save"  # You can extract the content to save from the response_text or generate it

        # Call the save_output_to_file function with the required parameters
        save_output_to_file(content_to_save, "D:\\JARVIS\\server\\public\\text", file_name)
