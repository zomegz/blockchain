...
def get_huggingface_key():
    ...
    if key is None:
        key = "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"  # Replace this with your actual Hugging Face API key
    return key

def get_openai_key():
    key = os.getenv("OPENAI_API_KEY")
    if key is None:
        key = "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"  # Replace this with your actual OpenAI API key
    return key
...
def save_output_to_file(output_text, output_directory, output_filename):
    import os
...
def send_request(data, headers):
    ...
    if use_completion:
        ...
        output_directory = "D:/JARVIS/server/public/text/"
        output_filename = "generated_output.txt"  # You can use a more meaningful filename related to the user request
        save_output_to_file(response_text, output_directory, output_filename)
...
def chitchat(messages, openaikey):
    data = {
        "messages": messages,
        "openaikey": openaikey,
    }
    try:
        response = send_request(data, {"Authorization": f"Bearer {openaikey}"})
        response_text = response.json()["choices"][0]["text"].strip()
...
def parse_task(context, input, openaikey):
...
def chat_huggingface(messages, openaikey = None, return_planning = False, return_results = False):
 ...
 # Check if the response text contains instructions to create a text file
    if "I have created a text file" in response_text:
        # Extract the file name and content from the response text
        import re
        file_name_match = re.search(r'named "(.+?)"', response_text)
        content_match = re.search(r'The content of the file is "(.+?)"', response_text)

        if file_name_match and content_match:
            file_name = file_name_match.group(1)
            content = content_match.group(1)

            # Save the content to the specified file
            output_directory = 'D:/JARVIS/server/public/text'
            save_output_to_file(content, output_directory, file_name)
